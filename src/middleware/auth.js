...
const hashPass = async function(req, res, next){
    try{
        if(!req.body.password) { return res.status(403).send({msg : "author password is required"})}
...
const auth = async (req,res,next)=>{
    try{
        ...
        if(!user) return res.status(403).send({status:false, msg:"invalid user!"});
        const hashPass = await bcrypt.compare(password,user.password);
        if(hashPass!==true){ 
            return res.status(403).send({status:false, msg:"Password Not Matche!"});
        }
        next();
    }catch (error) {
        res.status(500).send({status:false, msg : error.message})
...
const auth2 = async(req,res,next)=>{
    try {
        ...
        }else if(Object.keys(req.query).length===0) return res.status(403).send({status:false, msg :"Add Query Parameters"})
       
        if(id === null) return res.status(403).send({status:false, msg :" Data not found"})
        if(id.authorId != user._id ) return res.status(403).send({status:false, msg :"user unauthorized"})
...
const auth4 = async(req,res,next)=>{
    try {
        ...
        if(!decoding) return res.status(403).send({status:false,message:"Invalid token!"});
        const theUser = await Author.findById(decoding.userId);
        if(req.body.authorId != theUser._id) return res.status(403).send({status: false, msg : "Not valid a author!"})
        next()
    } catch (error) {
        res.status(500).send({status:false, error:error.message});
    }
